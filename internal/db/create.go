...
func CreatePost(ctx context.Context, body io.ReadCloser, username string) (*model.Post, error) {
	...
	// Might remove idk
	convertedContent["reaction"] = compiledRegex.FindString(convertedContent["reaction"].(string))

	if convertedContent["reaction"].(string) == "" {
		return nil, messages.ErrInvalid
	}
...
func CreateComment(ctx context.Context, body io.ReadCloser, username string, postID int) (*model.Comment, error) {
	...
	comment.Message = compiledRegex.FindString(comment.Message)
