...
func FindOnePostPipeline(postParam string) mongo.Pipeline {
	userPipeline := `
[
  { "$lookup": {
    "from": "likes",
  ...
  { "$lookup": {
	...
	hex, _ := primitive.ObjectIDFromHex(postParam)

	decoded := mongoPipeline(userPipeline)
	matchStage := bson.D{{"$match", bson.D{{"_id", hex}}}}
	totalPipeline := append(decoded, matchStage)

	fmt.Println(totalPipeline)

	return totalPipeline
}

// Thanks https://github.com/simagix/mongo-go-examples
func mongoPipeline(str string) []bson.D {
	var pipeline = []bson.D{}
